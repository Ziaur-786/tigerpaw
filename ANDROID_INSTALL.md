# Tigerpaw on Android (Termux)

A complete guide to running Tigerpaw on Android using Termux + proot Ubuntu.
...
Tigerpaw requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 3 — Clone Tigerpaw

```bash
git clone https://github.com/Gitlawb/tigerpaw.git
cd tigerpaw
npm install
npm link
```
...
### Step 6 — Build Tigerpaw

```bash
cd /data/data/com.termux/files/home/tigerpaw
bun run build
```

You should see:
```
✓ Built tigerpaw v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to Tigerpaw's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the Tigerpaw repo.
