/**
 * Tigerpaw startup screen — filled-block text logo with sunset gradient.
 * Called once at CLI startup before the Ink UI renders.
 *
 * Addresses: https://github.com/Gitlawb/tigerpaw/issues/55
 */
...
export function printStartupScreen(): void {
  ...
  // Gradient logo
  const allLogo = [...LOGO_TIGER, '', ...LOGO_PAW]
  ...
  out.push(`  ${DIM}${rgb(...DIMCOL)}BY${RESET} ${rgb(...CREAM)}AKASH RAHAMAN${RESET}`)
  out.push('')
  ...
  out.push(`${rgb(...BORDER)}\u255a${'\u2550'.repeat(W - 2)}\u255d${RESET}`)
  out.push(`  ${DIM}${rgb(...DIMCOL)}tigerpaw ${RESET}${rgb(...ACCENT)}v${MACRO.DISPLAY_VERSION ?? MACRO.VERSION}${RESET}`)
  out.push('')
