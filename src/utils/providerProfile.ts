...
export const PROFILE_FILE_NAME = '.tigerpaw-profile.json'
...
export async function buildStartupEnvFromProfile(options?: {
  ...
  return buildLaunchEnv({
    ...
    goal:
      options?.goal ??
      normalizeRecommendationGoal(processEnv.TIGERPAW_PROFILE_GOAL),
...
export async function applySavedProfileToCurrentSession(options: {
  ...
  const nextEnv = await buildLaunchEnv({
    ...
    goal: normalizeRecommendationGoal(processEnv.TIGERPAW_PROFILE_GOAL),
    processEnv: baseEnv,
    getOllamaChatBaseUrl,
