# Tigerpaw Local Agent Playbook

This playbook is a practical guide to run Tigerpaw with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, Tigerpaw starts directly.
...
Or let Tigerpaw recommend the best local model for your goal:
...
```powershell
Get-Content .\.tigerpaw-profile.json
```
...
```powershell
cd C:\Users\Lucas Pedry\Documents\tigerpaw\tigerpaw
bun run dev:profile
```
...
- Keep `.tigerpaw-profile.json` local (already gitignored).
